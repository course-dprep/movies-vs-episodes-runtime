...
## Research Method
To answer the research question, a regression analysis will be conducted. This approach is well suited to estimating the relationship between runtime and ratings for the content type (movie vs. TV episode) while treating release year as a covariate. An interaction term allows for testing whether the effect of runtime differs between movies and TV episodes. The model not only measures the strength and direction of these relationships, but also tests their statistical significance. If needed, additional exploratory analyses will be conducted, for example by grouping release years into broader periods or by checking non-linear patterns, to make sure the results are reliable.
